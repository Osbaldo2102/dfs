...
export default function ServiciosBarberiaPage() {
  ...
  const crearServicio = async (ev) => {
    ...
    try {
      ...
      setError('Error de red al intentar crear el servicio'); 
    } finally {
      setLoading(false);
    }
  };
    ...
    <main className="max-w-6xl mx-auto p-6 font-sans">
      ...
      <section className={esAdmin ? "grid grid-cols-1 gap-4" : "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8"}>
        {servicios.length > 0 ? (
          ...
          !loading && <p className="col-span-full text-center text-zinc-500 font-bold uppercase tracking-widest p-20">No hay servicios disponibles</p>
        )}
      </section>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center gap-4 mt-12">
          <button 
            ...
            className="bg-white px-6 py-3 rounded-xl shadow-sm font-bold disabled:opacity-30 border border-zinc-100"
            disabled={pagina === 1}
          > Anterior </button>
          <span className="flex items-center font-black text-zinc-400">Página {pagina} de {totalPages}</span>
          <button 
